eLoggingCharts = {
	...
	createNewCanvas : function(id) {
		var canvas = YUI3.Node.one('#chart_canvas_' + id);
		var newCanvas = YUI3.Node.create('<div id="chart_canvas_' + id + '" class="chartCanvas"></div>');
		canvas.replace(newCanvas);
	...
	configDownloadCallback : function(selectedTest, eventsTableWrapper) {
		...
		var eventsTable = YUI3.Node.create('<table cellpadding="0" cellspacing="0" border="0" class="display ' + testId + '" id="chart_EventsTable_' + eLoggingCharts.chartsCount + 
'"></table>');
	...
	getSeriesFromCSV : function(events, property, test) {
		...
		if (!events.series[property + '_ignoreDateStamps-' + eLoggingCharts.ignoreDateStamp]) {
			var series = [];
			events.series[property] = series;
		...
		for (i = 0; i < selections.length; i++) {
			...
			if (sel.test.config.conversionType == "CSV") {
				...
				task.done(function(data) {
					...
					// this.selectedEvent.csvData = $.csv.toObjects(data);
				});
	...
	testVAVBTests : function() {
		var fileNames = [];
		for (var i = 0; i < eLoggingCharts.eLoggingTests.length; i++) {
			testid = eLoggingCharts.eLoggingTests[i];
			if (testid.indexOf("VA") > -1 && testid.indexOf("T2") > -1) {
				fileNames.push(eLoggingCharts.testsRootFolder + testid + '/config.js');
			}
		}

		YUI().Get.script(fileNames, {
			onEnd : function(e) {

				for (var i = 0; i < eLoggingCharts.eLoggingTests.length; i++) {
					testid = eLoggingCharts.eLoggingTests[i];
					if (testid.indexOf("VA") > -1 && testid.indexOf("T2") > -1) {
						var vaStorefronts = [ "VAVA", "VAVB" ];
						var status = [];

						for (var j = 0; j < vaStorefronts.length; j++) {
							var shop = false;
							var extras = false;
							var book = false;
							var type = "VA_SERV_TP_" + vaStorefronts[j];
							servTp = eLoggingCharts.tests[testid].type[type];
							if (servTp) {
								if (servTp["AirSearchApi.search"]) {
									shop = true;
								}
								if (servTp["TravelExtrasApi.shopTravelExtras"]) {
									extras = true;
								}
								if (servTp["BookingApi.purchase"]) {

									book = true;
								}

							}
							status.push(shop, extras, book);
						}
